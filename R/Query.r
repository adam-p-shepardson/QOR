...
#' @param units_per_batch number of units to geocode in each batch (conservative default: 4000)
...
query <- function(units = NULL, street = "street", city = "city", state = "state", state_shape = NULL, units_per_batch = 4000, year = NULL) {
    ...
    state_shape <- state_shape %>%
        sf::st_make_valid() %>%
        sf::st_transform(., crs = sf::st_crs(units)) # sets the two objects to the same coordinate reference system.
